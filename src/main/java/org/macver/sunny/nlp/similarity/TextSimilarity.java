...
public final class TextSimilarity {
    ...
    private TermFrequencyInverseDocumentFrequency tfIdf = new TermFrequencyInverseDocumentFrequency();
    private Map<String, double[]> vectors = null;

    /**
     * Constructs a new instance using {@link TokenizerImpl} for splitting text into tokens.
     */
    ...
    @NotNull
    @SuppressWarnings("WeakerAccess")
    public Map<Double, String> getSimilarDocuments(String id, int limit) {
        ...
        Map<Double, String> bestMatches = new HashMap<>();
        for(int i = 0; i < limit && !similarity.isEmpty(); i++) {
            ...
            bestMatches.put(bestMatch, bestMatchId);
    ...
    @NotNull
    @SuppressWarnings("WeakerAccess")
    public Map<Double, String> getSimilarDocuments(String id) {
        return getSimilarDocuments(id, 25);
    }

    private double cosineSimilarity(@NotNull double[] v1, double[] v2) {
